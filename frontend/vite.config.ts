...
// Site metadata is owned by the Figma Make harness at the repo root, one level up.
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      // `.figma/make/deploy` runs `pnpm run build` at the repo root and then
      // publishes `<repo>/dist`, so the bundle has to land outside this package.
